# scripts/similarity_search_final_fixed.py
"""
İç Mekan Benzerlik Arama Sistemi - Benzerlik Arama (Son Düzeltme)
Veri tipi sorunları çözüldü
"""
...
class SimilaritySearcher:
    ...
    def load_models(self):
        ...
        if os.path.exists(features_path):
            self.features = np.load(features_path).astype(np.float64)  # double tipine çevir
    ...
    def _create_style_clusters(self):
        ...
        # K-Means kümeleme uygula (features zaten double tipinde)
        ...
        # Küme-kategori ilişkisi (JSON uyumlu tiplerle)
        ...
        for cluster_id in unique_clusters:
            ...
            # NumPy tiplerini Python tiplerene çevir
            cluster_category_map[str(cluster_id)] = {  # int32 -> str
                'dominant_category': dominant_category,
                'size': int(cluster_counts[cluster_id]),  # int32 -> int
                'categories': {k: int(v) for k, v in cluster_categories.to_dict().items()}  # int64 -> int
            }
    ...
    def extract_query_features(self, query_image_path):
        ...
        if self.pca_model is not None:
            # PCA uygula ve double tipine çevir
            features_pca = self.pca_model.transform([raw_features]).astype(np.float64)
            
            # UMAP uygula (varsa)
            if self.umap_model is not None:
                final_features = self.umap_model.transform(features_pca).astype(np.float64)
            else:
                final_features = features_pca
        else:
            final_features = raw_features.reshape(1, -1).astype(np.float64)
    ...
    def calculate_similarity_scores(self, query_features, method='cosine'):
        ...
        # Query features'ı double tipine çevir
        query_features = np.array(query_features).astype(np.float64)
    ...
    def search_similar_images(self, query_image_path, k=10, category_filter=None, 
        ...
        # 2. Sorgu görseli stil kümesini bul (double tipinde)
        query_features_reshaped = query_features.reshape(1, -1).astype(np.float64)
        query_cluster = self.kmeans_model.predict(query_features_reshaped)[0]
    ...
    def get_similar_by_style(self, query_image_path, k=10):
        ...
        # Sorgu stil kümesini bul (double tipinde)
        query_features_reshaped = query_features.reshape(1, -1).astype(np.float64)
        query_cluster = self.kmeans_model.predict(query_features_reshaped)[0]
